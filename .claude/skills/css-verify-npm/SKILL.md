...
- Node.js 18.3.0 以上
- インターネット接続（初回実行時に `@svjunic/css-diff` をダウンロード）
- または事前に `npm install -g @svjunic/css-diff` でグローバルインストール済み
...
| `npx: command not found` | Node.jsが入っていない | Node.js 18.3.0以上をインストール |
| `npm ERR! 404` | パッケージが見つからない | `@svjunic/css-diff` がpublishされているか確認 |
